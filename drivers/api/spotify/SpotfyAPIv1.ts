...
export class SpotifyApiV1 implements ISpotifyAPI {
  ...
  ): Promise<SpotifyTokenResponseJSON> => {
  ...
  public getCurrentlyPlaying =
    async (): Promise<SpotifyCurrentlyPlaying | null> => {
      ...
      const fetchData = async () => {
        ...
        if (res.ok && data) {
          const transformed = this.transformCurrentlyPlayingResponseToJSON(data)
      ...
      switch (res.status) {
        ...
        case constants.HTTP_STATUS_NO_CONTENT:
          // const lastPlayed =
          // await this.connection.getParsed<SpotifyCurrentlyPlaying>(
          //   DatabaseKeys.SPOTIFY_LAST_PLAYED
          // )
          const lastPlayed = await this.connection.get<SpotifyCurrentlyPlaying>(
            DatabaseKeys.SPOTIFY_LAST_PLAYED
          )
          if (lastPlayed) {
            return {
  ...
  public getLibrary = async ({
    limit,
    offset = 0,
  }: GetSpotifyLibraryInput): Promise<SpotifyLibrary | null> => {
    const query = new URLSearchParams()
   ...
   */
  public getPlaylist =
    async ({}: GetSpotifyPlaylistInput): Promise<SpotifyPlaylist | null> => {
      const playlist_id = serverEnv.SPOTIFY_PLAYLIST_ID
      const query = new URLSearchParams()
      const res = await this.fetch(
